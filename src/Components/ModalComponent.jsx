...
const ModalComponent = ({ projectNumber }) => {
  const { isDarkMode, theme, modalStates, handleProjectModal } =
    usePorfolioContext();
  const currentProject = DataModal.find(
    (project) => project.id === projectNumber
  );
    ...
    <div>
      {modalStates[projectNumber] && (
        ...
        >
          <button
            className="close"
            onClick={() => handleProjectModal(projectNumber)}
          >
            X
          </button>
          <div className="megaCOntainer">
            <div className="modalContent">
              <h1
                style={{ color: isDarkMode ? theme.detallenos2D : theme.textD }}
              >
